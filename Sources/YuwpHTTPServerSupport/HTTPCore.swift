...
public enum HTTPServerLimits {
    public static let defaultMaxBodySize = 100 * 1024 * 1024
}
...
public final class HTTPStreamWriter: @unchecked Sendable {
    private let writeChunk: @Sendable (Data) -> Void
    private let writeFinalChunk: @Sendable () -> Void
    private let lock = NSLock()
    private var finished = false

    public init(
        writeChunk: @escaping @Sendable (Data) -> Void,
        writeFinalChunk: @escaping @Sendable () -> Void
    ) {
        self.writeChunk = writeChunk
        self.writeFinalChunk = writeFinalChunk
    }
...
public struct HTTPResponse: Sendable {
    ...
    public let body: Data
    public let stream: (@Sendable (HTTPStreamWriter) -> Void)?
    ...
    public init(status: Int, contentType: String, stream: @escaping @Sendable (HTTPStreamWriter) -> Void) {
        self.status = status
        self.contentType = contentType
        self.body = Data()
        self.stream = stream
...
public func binaryResponse(status: Int, body: Data, contentType: String) -> HTTPResponse {
    HTTPResponse(status: status, contentType: contentType, body: body)
}

public func streamingResponse(
    status: Int = 200,
    contentType: String,
    _ stream: @escaping @Sendable (HTTPStreamWriter) -> Void
) -> HTTPResponse {
    HTTPResponse(status: status, contentType: contentType, stream: stream)
}
