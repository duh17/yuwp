// NativeASR — Qwen3-ASR configuration
// Parses config.json from HuggingFace Qwen3-ASR model directory.
...
// MARK: - Text Decoder Config (Qwen3 LLM)
...
// MARK: - Thinker Config (nested HF wrapper)

struct ThinkerConfig: Codable, Sendable {
...
public struct Qwen3ASRConfig: Codable, Sendable {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        if let q = quantizationConfig {
            try container.encode(q, forKey: .quantizationConfig)
        }
    }

    /// Load config from a directory containing config.json
    public static func load(from directory: URL) throws -> Qwen3ASRConfig {
        let url = directory.appendingPathComponent("config.json")
        let data = try Data(contentsOf: url)
        return try JSONDecoder().decode(Qwen3ASRConfig.self, from: data)
    }
}
...
public enum Qwen3ASRError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case let .audioLoadFailed(msg): return "Audio load failed: \(msg)"
        case let .modelLoadFailed(msg): return "Model load failed: \(msg)"
        case let .tokenizerLoadFailed(msg): return "Tokenizer load failed: \(msg)"
