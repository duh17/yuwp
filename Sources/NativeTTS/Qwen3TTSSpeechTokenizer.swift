...
final class Qwen3TTSSpeechTokenizerEncoder: Module {
    ...
    @ModuleInfo var encoder: TTSCodecSeanetEncoder
    @ModuleInfo(key: "encoder_transformer") var encoderTransformer: TTSCodecProjectedTransformer
    @ModuleInfo var downsample: TTSCodecConvDownsample1d
    @ModuleInfo var quantizer: TTSCodecSplitResidualVectorQuantizer
    ...
    init(config: Qwen3TTSTokenizerEncoderConfig, validNumQuantizers: Int) {
        ...
        let seanetCfg = TTSCodecSeanetConfig(
        ...
        _encoder.wrappedValue = TTSCodecSeanetEncoder(cfg: seanetCfg)

        let transformerCfg = TTSCodecTransformerConfig(
        ...
        let projectedTransformer = TTSCodecProjectedTransformer(
            cfg: transformerCfg,
            inputDim: config.hiddenSize,
            outputDims: [config.hiddenSize]
        )
        _encoderTransformer.wrappedValue = projectedTransformer
        _downsample.wrappedValue = TTSCodecConvDownsample1d(
            stride: downsampleStride,
            dim: config.hiddenSize,
            causal: config.useCausalConv
        )
        _quantizer.wrappedValue = TTSCodecSplitResidualVectorQuantizer(
